...
internal class XmlRpcStructContextSourceGenerator : IIncrementalGenerator
{
  ...
  private string GenerateContextClass(XmlRpcContextInfo contextInfo, SourceProductionContext productionContext)
  {
    ...
    XmlRpcConverterGenerator converterGenerator = new XmlRpcConverterGenerator(textWriter, contextInfo, contextInfo.GetSerializedTypes());
    ...
    for (int i = contextInfo.Parents.Count - 1; i >= 0; i--)
    {
      ...
      switch (syntaxNode)
      {
        ...
        case BaseNamespaceDeclarationSyntax:
          ...
          break;
        default:
          throw new Exception($"Unexpected SyntaxNode {contextInfo.GetType().FullName}");
      }
    }
