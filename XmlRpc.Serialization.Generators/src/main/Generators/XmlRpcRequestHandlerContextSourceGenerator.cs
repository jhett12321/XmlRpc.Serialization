...
[Generator]
internal sealed class XmlRpcRequestHandlerContextSourceGenerator : IIncrementalGenerator
{
  private const string ClassAttributeTypeName = "XmlRpc.Serialization.Attributes.XmlRpcRequestHandlerAttribute";
  private const string ClassAttributeClassName = "XmlRpcRequestHandlerAttribute";
  ...
  private XmlRpcContextInfo? GetSemanticTargetForGeneration(GeneratorAttributeSyntaxContext context)
  {
    ...
    XmlRpcTypeInfo serializedType = new XmlRpcTypeInfo((INamedTypeSymbol)context.TargetSymbol);
  ...
  private string GenerateContextClass(XmlRpcContextInfo contextInfo, SourceProductionContext productionContext)
  {
    ...
    XmlRpcConverterGenerator converterGenerator = new XmlRpcConverterGenerator(textWriter, contextInfo, contextInfo.GetSerializedTypesFromMethods());
    XmlRpcRequestMethodGenerator methodGenerator = new XmlRpcRequestMethodGenerator(textWriter, contextInfo);
    ...
    textWriter.WriteLine("using XmlRpc.Serialization.Exceptions;");
    ...
    {
      converterGenerator.GenerateConverters("private");
      methodGenerator.GenerateHandleRequestMessage();
    }
    textWriter.EndScope();
